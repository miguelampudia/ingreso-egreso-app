...
export class RegisterComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.registroForm = this.fb.group({ nombre: ['', Validators.required], correo: ['', [Validators.required, Validators.email]], password: ['', Validators.required] });

    this.uiSubcripcion = this.store.select('ui').subscribe(ui => this.cargando = ui.isLoading);
  }
